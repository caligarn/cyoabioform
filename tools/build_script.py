...
The script lives in `script/versions/`, one dated file per version, with
`script/versions.json` saying which is current. The current document is the
...
dependencies and CI should stay that way) and writes two things:
    ...
    script.html                     the current version as a page, one anchor per
                                    branch, scene and prompt, so a board row can
                                    link to the exact scene it covers
    ...
    script/versions/<date>.html     a page per superseded .docx version, so an
                                    earlier version can be reviewed the same way

Every page carries its version date at the top and a version history at the
end, with the earlier files kept in the repo so they can be opened and
compared. Adding a version: drop the dated file in script/versions/, add an
entry at the top of script/versions.json, rebuild, commit all of it.

    python3 tools/build_script.py           # rebuild both
    python3 tools/build_script.py --check   # fail if either is stale (CI)

Edit the document and the version log, not the outputs. Formatting in the document is what the
...
VERSIONS = ROOT / "script" / "versions.json"
PAGE = ROOT / "script.html"
DATA = ROOT / "script" / "production-script.json"
ARCHIVE = ROOT / "script" / "versions"
...
def versions():
    """The version log, newest first. The first .docx entry is the current one."""
    log = json.loads(VERSIONS.read_text(encoding="utf-8"))["versions"]
    log.sort(key=lambda v: v["date"], reverse=True)
    for v in log:
        v["path"] = ROOT / v["file"]
        v["when"] = longdate(v["date"])
    return log
...
def render(branches, log, version):
    """version is the entry being rendered; the current one is log[0]'s docx."""
    current = next(v for v in log if v["format"] == "docx")
    is_current = version is current
    rel = "" if is_current else "../../"     # archived pages live two levels down
    ...
    o.append('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">\n'
             ...
             f'<link rel="icon" href="{rel}assets/img/favicon.svg" type="image/svg+xml">\n'
             f'<link rel="stylesheet" href="{rel}assets/css/site.css">\n'
             f'<link rel="stylesheet" href="{rel}assets/css/script.css"></head>'
             f'<body class="script{"" if is_current else " old"}">\n'
             f'<!-- Generated by tools/build_script.py from {version["file"]} '
             f'(version of {version["date"]}). Edit the document, then rebuild. -->\n')
    ...
    o.append('<div class="shell"><nav class="side">\n'
             ...
             f'<a href="{rel}index.html" class="ext"><span class="n">←</span>Contributor hub</a>\n'
             f'<a href="{rel}sample.html" class="ext"><span class="n">▶</span>Play the sample</a>\n'
             '<a href="#contents" data-nav="contents"><span class="n">§</span>Contents</a>\n'
             '<a href="#versions" data-nav="versions"><span class="n">ver</span>Versions</a>\n'
             '<div class="grp">Branches</div>\n')
    for b in branches:
        o.append(f'<a href="#{b["id"]}" data-nav="{b["id"]}"><span class="n">'
                 f'{esc(label(b))}</span>{esc(nice(b["title"]))}</a>\n')
    o.append('</nav><main>\n<div class="mobbar">\n'
             f'<a href="{rel}index.html">Hub</a>\n'
    ...
    # top
    if is_current:
        banner = (f'<div class="version"><b>Current version · {esc(version["when"])}</b>'
                  f'<span>{esc(version["title"])}. This is the text the board and the map '
                  'are cut from. Earlier versions are kept below, unchanged, for review.</span>'
                  '<a href="#versions">Version history</a></div>\n')
    else:
        banner = (f'<div class="version old"><b>Superseded · version of {esc(version["when"])}</b>'
                  f'<span>{esc(version["title"])}. Kept for review. The current version is '
                  f'{esc(current["when"])}.</span>'
                  f'<a href="{rel}script.html">Read the current version</a></div>\n')
    o.append('<header class="top">\n'
             '<div class="kicker">Machine Cinema · production script · '
             f'version of {esc(version["date"])}</div>\n'
    ...
    o.append(
             ...
             '</div></div>\n'
             f'<p class="gen">Generated from <code>{esc(version["file"])}</code> '
             'by <code>tools/build_script.py</code>. Edit the document, not this page. '
             + ('The same content is available as data in '
                '<a href="script/production-script.json">production-script.json</a>.'
                if is_current else
                'This is an archived version; its data file is not kept.')
             + '</p>\n</header>\n')
    ...
    o.append('<section id="versions"><div class="eyebrow">Versions</div>'
             '<h2 class="sec">Every version, newest first</h2>\n'
             '<p class="lede">Each version is kept in the repo as the file it arrived as, '
             'so an earlier draft can be opened and compared against the current one. '
             'The date is the version date, not the day the page was built.</p>\n'
             '<div class="vlist">\n')
    for v in log:
        here = v is version
        tags = ('<span class="pill go">current</span>' if v is current else
                '<span class="pill">superseded</span>')
        if here and not is_current:
            tags += '<span class="pill hot">this page</span>'
        links = [f'<a href="{rel}{esc(v["file"])}">Open the {esc(v["format"])}</a>']
        if v["format"] == "docx" and v is not current:
            links.append(f'<a href="{rel}script/versions/{v["date"]}.html">Read as a page</a>')
        if v is current and not is_current:
            links.append(f'<a href="{rel}script.html">Read as a page</a>')
        if v.get("source"):
            links.append(f'<a href="{esc(v["source"])}" target="_blank" rel="noopener">'
                         f'{esc(v.get("source_label", "Source"))}</a>')
        o.append(f'<div class="vrow{" here" if here else ""}">'
                 f'<div class="vdate">{esc(v["when"])}</div>'
                 f'<div><b>{esc(v["title"])}</b> {tags}'
                 f'<p>{esc(v.get("notes", ""))}</p>'
                 f'<div class="vlinks">{" · ".join(links)}</div></div></div>\n')
    ...
    o.append('<footer>Generated from the production script document. '
             ...
             '</div></main></div>\n'
             f'<script src="{rel}assets/js/script.js" defer></script></body></html>\n')
    return "".join(o)


def data(branches, log, version):
    out = {
        "source": version["file"],
        "version": {"date": version["date"], "title": version["title"]},
        "versions": [{k: v[k] for k in ("date", "title", "file", "format", "source", "notes")
                      if k in v} for v in log],
...
def main():
    log = versions()
    missing = [v["file"] for v in log if not v["path"].is_file()]
    if missing:
        sys.exit("versions.json names files that are not in the repo: " + ", ".join(missing))
    current = next(v for v in log if v["format"] == "docx")

    outputs = {}                                   # path -> wanted text
    branches = parse(current["path"])
    outputs[PAGE] = render(branches, log, current)
    outputs[DATA] = data(branches, log, current)
    for v in log:
        if v["format"] == "docx" and v is not current:
            outputs[archive_page(v)] = render(parse(v["path"]), log, v)

    if "--check" in sys.argv:
        stale = [str(p.relative_to(ROOT)) for p, want in outputs.items()
                 if not p.is_file() or p.read_text(encoding="utf-8") != want]
        if stale:
            sys.exit("stale, rebuild with tools/build_script.py: " + ", ".join(stale))
        print(f"script.html, production-script.json and {len(outputs) - 2} archived "
              f"page(s) match the documents (current version {current['date']})")
        return
    ...
    print(f"wrote script.html ({len(outputs[PAGE]) / 1024:.0f} KB) and "
          f"script/production-script.json ({len(outputs[DATA]) / 1024:.0f} KB) "
          f"from the {current['date']} version: "
          f"{len(branches)} branches, {scenes} scenes, {prompts} prompts; "
          f"{len(outputs) - 2} archived page(s)")
